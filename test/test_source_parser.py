...
class BasicParamsCase(TestCase):
    ...
    def test_no_source_parseracing(self):
        self.assertListEqual([["int myFunc", "bool myVarA", "String myVarB"]], source_parser.parse_signature("int myFunc(bool myVarA,String myVarB){", 2))

    def test_everywhere_source_parseracing(self):
        self.assertListEqual([["int myFunc", "bool myVarA", "String myVarB"]], source_parser.parse_signature("  int myFunc (  bool myVarA,  String myVarB  ) {  ", 2))
...
class OpenScadCase(TestCase):
    ...
    def test_constructor_initializer(self):
        self.assertListEqual([["void VBORenderer::resize", "int w", "int h"]], source_parser.parse_signature("VBORenderer::VBORenderer()\n  : Renderer()\n{\n}\n\nvoid VBORenderer::resize(int w, int h)\n{\n  Renderer::resize(w, h);\n}"))
# TODO array types
