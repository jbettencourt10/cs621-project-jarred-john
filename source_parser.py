...
def parse_signature(source_code, max_params=10):
    ...
    declaration = f"(?:const{space})?{type_name}{any_space}[&\*]?{space}[&\*]?{variable_name}"
    parameter = lambda capture: f"(?:{any_space}{('(' if capture else '') + declaration + (')' if capture else '')},?{any_space})"
    regex_string = f"{beginning_characters}({declaration}){any_space}\\({(parameter(True) + '?')*(max_params)}{parameter(False)}*\\){any_space}(?:const{any_space})?\\{{.*"
